...
static inline unsigned char randchar() { //return 255;
...
__m256i left_border[NTASKS];

//descarga slots [from,to), actualiza h (salvo borde derecho) y devuelve cantidad de slots activos en result.
void process(int from, int to, int tasknum, Manna_Array __restrict__ h, unsigned int *result)
{
	#ifdef DEBUG
...
unsigned int descargar(Manna_Array __restrict__ h, Manna_Array __restrict__ dh)
{
	...
	unsigned int activos[NTASKS];
	
	#pragma omp parallel
    #pragma omp single nowait
    {
	//lanzo una task por cada pedazo de for
	for (int i=NSIMD; i < N-NSIMD; i += STEP) {
		...
		#pragma omp task shared(activos,nroactivos)
		{
	...
	//unir left_borders a h
	for(int t=0; t<NTASKS; ++t){
		int slot = NSIMD + (t+1)*STEP - 1;
		
		#ifdef DEBUG
		cout<<"Slot "<<slot<<" task: "<<t<<". activos "<<activos[t]<<endl;
		#endif
		
		nroactivos -= (h[slot]>1);
		nroactivos -= (h[slot+1]>1);
		h[slot] += _mm256_extract_epi32(left_border[t],0);
		h[slot+1] += _mm256_extract_epi32(left_border[t],1);
 ...
 * TODO:
 *   Chequear si anda!!!
 *   Pulir, se pueden optimizar un par de fors capaz.
 *   Buscar valores óptimos para NTASKS.
