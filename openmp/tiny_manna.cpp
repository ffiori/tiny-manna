...
unsigned int descargar(Manna_Array __restrict__ h, Manna_Array __restrict__ dh)
{
	...
	#pragma omp parallel for shared(h,dh) schedule(static)
	for (i=NSIMD; i < N-NSIMD; i+=NSIMD) {
		...
		if(activity){
			...
			if(!_mm256_testz_si256(left_to_store,left_to_store)){
				//~ #pragma omp critical(dh_io) //TODO usar locks, se pueden bloquear solamente secciones contiguas
...
*/
	...
	for(int i=0; i<N/NSIMD; ++i) omp_init_lock(&LOCK[i]);
