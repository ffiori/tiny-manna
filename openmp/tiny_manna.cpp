...
void printdh(Manna_Array dh){
	cout<<"dh[]: ";
	for(int i=0; i<SIZE; i++) cout<<dh[i]<<" ";
	cout<<endl;
}
#endif
...
unsigned int descargar(Manna_Array __restrict__ h, Manna_Array* __restrict__ dh)
{
	...
	#pragma omp parallel shared(dh,h)
	{
	...
	for (i=NSIMD; i < N-NSIMD; i+=NSIMD) {
		...
		if(activity){
			...
			_mm256_store_si256((__m256i *) &h[i],slots); //OJO!!!
			
			//~ if(!_mm256_testz_si256(left_to_store,left_to_store)){
				//~ #pragma omp critical(dh_io)
				{
					slots = _mm256_loadu_si256((__m256i *) &dh[myID][i-1]);
					slots = _mm256_add_epi32(slots, left_to_store);
					_mm256_storeu_si256((__m256i *) &dh[myID][i-1], slots);
				}
			//~ }
			...
			//~ if(!_mm256_testz_si256(right_to_store,right_to_store)){
				//~ #pragma omp critical(dh_io)
				{
					slots = _mm256_loadu_si256((__m256i *) &dh[myID][i+7]);
					slots = _mm256_add_epi32(slots, right_to_store);
					_mm256_storeu_si256((__m256i *) &dh[myID][i+7], slots);
				}
			//~ }
		}
	}
