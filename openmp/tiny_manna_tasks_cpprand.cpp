...
default_random_engine generator[MAX_THREADS*CACHECHK];
...
void randinit() {
	...
	for(int i=0; i<MAX_THREADS; ++i) 
		generator[i*CACHECHK] = default_random_engine(SEED ? SEED : rd());
		//~ seeds[i*CACHECHK]=SEED ? SEED : rd();
	//~ srand(SEED ? SEED : time(NULL));
}

static inline bool randbool() {
	//~ return rand_r(&seeds[omp_get_thread_num()*CACHECHK]); //return 1;
	uniform_int_distribution<int> distribution(0,1);
	return distribution(generator[omp_get_thread_num()*CACHECHK]);
}

static inline unsigned char randchar() { 
	//~ return rand_r(&seeds[omp_get_thread_num()*CACHECHK]); //return 0;
